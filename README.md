...
### 🔒 军工级本地保险箱 (Vault)
- **本地零信任加密**：内置支持 PBKDF2 和 AES-GCM 高强度加密的独立密码保险箱，数据仅在前端通过您的主密码动态解锁，后端无法窥探明文内容。
- **防内存泄露与锁定**：闲置超时自动上锁，保障离开设备时的绝对隐私安全。

### 🛠️ 现代化工程体验
- **安全指令防护**：AI 在执行删除、修改等高风险操作前，会主动暂停推理链并弹出富文本卡片请求用户二次确认 (Human-in-the-loop)。
- **斜杠快捷菜单**：原生多语言且极具美感的 `/` 快捷指令列表，轻松呼出搜索、网页抓取等核心动作。
- **桌面级工具箱**：内置开发者刚需工具，包括 AI 翻译、Base64 / URL 编解码器、时间戳与颜色转换等。
- **多语言与跨平台**：内置中繁英多国语言一键切换，采用自适应暗色模式。提供轻量级 SQLite 支持以及开箱即用的 Docker 部署镜像。

## 技术栈

| 模块 | 技术 |
| --- | --- |
| 前端 UI | Vue 3, Tailwind CSS, TypeScript, Vue Router |
