export default {
  common: {
    save: "Save",
    cancel: "Cancel",
    confirm: "Confirm",
    delete: "Delete",
    edit: "Edit",
    success: "Success",
    error: "Error",
  },
  auth: {
    checkingStatus: "Checking login status...",
    loginDesc: "Manage your bookmarks and AI configurations after logging in.",
    defaultAccountHint: "Default deployment account: admin / linka123456. Please change it in settings after logging in.",
    username: "Username",
    password: "Password",
    passwordPlaceholder: "Enter password",
    hidePassword: "Hide password",
    showPassword: "Show password",
    rememberUsername: "Remember username",
    autoLogin: "Auto login",
    loginBtn: "Login",
  },
  settings: {
    title: "System Settings",
    backToHome: "Back to Home",
    tabs: {
      general: "General",
      account: "Account & Security",
      categories: "Categories",
      bookmarks: "Bookmarks",
      ai: "AI Assistant",
    },
    general: {
      title: "General Settings",
      desc: "Manage basic system display and behavior",
      language: "Interface Language",
      languageHint: "Select your preferred interface language",
      aiLanguage: "Linka AI Language",
      aiLanguageHint: "Select the default language the AI assistant uses to talk to you",
      weatherTitle: "Weather Widget",
      weatherDesc: "Display weather info on the top right (Powered by WeatherAPI)",
      weatherApiKey: "WeatherAPI Key",
      weatherApiKeyPlaceholder: "Enter your Weather API Key",
      weatherLocation: "City / Location",
      weatherLocationPlaceholder: "e.g., London, New York, or leave empty for auto:ip",
      weatherShowDate: "Display Date & Day",
      weatherDateFormat: "Date Format",
      weatherDateFormat_full: "Full (Y/M/D Weekday Time)",
      weatherDateFormat_long: "Long (Y/M/D Time)",
      weatherDateFormat_short: "Short (M/D Time)",
      weatherDateFormat_time: "Time Only (Time)",
      searchTitle: "Web Search",
      searchDesc: "Allow AI assistant to search the web and fetch page content for real-time information",
      searchEngine: "Search Engine",
      searchApiKey: "API Key",
      searchApiKeyPlaceholder: "Enter search engine API key",
      searchBaseUrl: "SearXNG URL",
      searchBaseUrlPlaceholder: "e.g. https://searxng.example.com",
      searchMaxResults: "Max Results",
    },
    account: {
      title: "Account & Security",
      desc: "Current account: {username}. Change default account and password to reduce security risks.",
      logout: "Log out",
      avatar: "Avatar",
      uploadAvatar: "Upload Avatar",
      saving: "Saving...",
      upload: "Upload",
      clear: "Clear",
      readAvatarFailed: "Failed to read avatar.",
      username: "Username",
      currentPassword: "Current Password",
      currentPasswordPlaceholder: "Required before saving changes",
      newPassword: "New Password",
      newPasswordPlaceholder: "Leave blank to keep unchanged",
      confirmPassword: "Confirm New Password",
      confirmPasswordPlaceholder: "Enter new password again",
      saveAccount: "Save Account",
    },
    categories: {
      title: "Category Management",
      desc: "AI will prioritize selecting from these categories. If a category is deleted, related bookmarks will be moved to 'Uncategorized'.",
      newCategoryPlaceholder: "New category name",
      add: "Add",
      dragSort: "Drag to sort",
      save: "Save",
      delete: "Delete",
    },
    bookmarks: {
      title: "Bookmark Management",
      desc: "Modify the name, URL, icon, and category of collected bookmarks here.",
      addBookmark: "Add Bookmark",
      edit: "Edit",
      url: "URL",
      titleField: "Title",
      optionalAuto: "Optional, will auto-fetch if left blank",
      icon: "Icon",
      iconPlaceholder: "Optional, supports online URL or local upload",
      upload: "Upload",
      readIconFailed: "Failed to read image.",
      category: "Category",
      aiAuto: "AI Auto Select",
      showOnHome: "Show on Home",
      addToLinka: "Add to Linka",
      editBookmarkTitle: "Edit Bookmark",
      summary: "Summary",
      summaryPlaceholder: "Optional, used for bookmark card summary; leave blank for 'No summary'",
      iconLink: "Icon URL",
      uncategorized: "Uncategorized",
      saveChanges: "Save Changes",
    },
    ai: {
      title: "Provider Management",
      desc: "Manage your AI service providers",
      modelCount: "{count} models",
      enabled: "Enabled",
      disabled: "Disabled",
      addProvider: "Add Provider",
      deleteProvider: "Delete Provider",
      dragToSort: "Drag to sort",
      apiKeyNotSet: "API Key not set",
      apiKeySetText: "API Key configured",
      defaultModel: "Default Model: ",
      none: "None",
      basicInfo: "Basic Info",
      providerName: "Provider Name",
      enableStatus: "Enable Status",
      apiConfig: "API Configuration",
      apiFormat: "API Format",
      formatOpenAI: "OpenAI Chat Completion",
      formatAnthropic: "Anthropic Messages",
      temperature: "Temperature",
      temperatureHint: "Controls randomness in model generation. Higher values are more creative, lower values are more precise.",
      baseUrl: "Base URL",
      apiKey: "API Key",
      loading: "Loading...",
      enterNewKey: "Enter new API key to overwrite",
      enterKey: "Enter API key",
      unlockKey: "Unlock to view/edit",
      lockKey: "Lock",
      copy: "Copy",
      modelList: "Model List",
      modelListDesc: "Drag models to adjust priority. The first model is used as default.",
      addModel: "Add Model",
      default: "Default",
      vision: "Vision",
      contextPrefix: "Context: ",
      tokenSuffix: " tokens",
      testConnection: "Test Connection",
      edit: "Edit",
      delete: "Delete",
      close: "Close",
      lastSaved: "Last saved: ",
      reset: "Reset Changes",
      saveConfig: "Save Configuration",
      editModelTitle: "Edit Model",
      addModelTitle: "Add Model",
      modelName: "Model Name",
      maxTokens: "Max Tokens",
      supportsVision: "Supports Vision",
      visionHint: "When enabled, this model can receive image/video attachments for understanding.",
      cancel: "Cancel",
      save: "Save",
      confirmDelete: "Confirm Deletion",
      deleteWarning: "Are you sure you want to delete this provider? All related model configurations and API keys will be cleared. This action cannot be undone.",
    }
  },
  assistant: {
    summon: "Summon Linka AI",
    dragResize: "Drag to resize width",
    dragResizeAria: "Drag to resize assistant panel width",
    history: "History",
    newConversation: "New Conversation",
    close: "Close",
    cancel: "Cancel",
    searchHistory: "Search history...",
    done: "Done",
    manage: "Manage",
    emptyHistory: "No history yet",
    selected: "Selected ",
    items: " items",
    batchDelete: "Batch Delete",
    welcome: "Hello! I am Linka AI",
    thinking: "Thinking...",
    thoughtDone: "Thought Process",
    processingAria: "Linka AI is processing",
    inputPlaceholder: "Ask anything...",
    removeAttachment: "Remove attachment",
    uploadAttachment: "Upload image, video, or attachment",
    defaultModel: "Default Model",
    voiceInput: "Voice Input",
    send: "Send (Enter)",
    stop: "Stop generating",
    effort: {
      none: "Off",
      default: "Default",
      low: "Low",
      medium: "Medium",
      high: "High",
      max: "Max"
    },
    commands: {
      addBookmark: "/bookmark",
      addBookmarkDesc: "Bookmark a specific URL",
      delBookmark: "/delete-bookmark",
      delBookmarkDesc: "Delete a specific bookmark",
      addCategory: "/category",
      addCategoryDesc: "Create a new category",
      delCategory: "/delete-category",
      delCategoryDesc: "Delete a specific category",
      searchWeb: "/search",
      searchWebDesc: "Search the web for real-time information",
      fetchWeb: "/fetch",
      fetchWebDesc: "Fetch and read the content of a specific URL"
    }
  },
  library: {
    openLink: "Open Link",
    noSummary: "No summary",
    pin: "Pin",
    edit: "Edit",
    restore: "Restore",
    archive: "Archive",
    delete: "Delete",
    deleteConfirm: "Are you sure you want to delete this bookmark?",
    emptyHomeTitle: "No frequent bookmarks on home",
    emptyHomeDesc: "Enable 'Show on Home' when adding or editing a bookmark.",
    emptyCategoryTitle: "No bookmarks in this category",
    emptyCategoryDesc: "Switch to another category or adjust bookmark categories in settings."
  },
  topbar: {
    themeLight: "Appearance: Light Mode (Click to switch to Dark Mode)",
    themeDark: "Appearance: Dark Mode (Click to switch to Light Mode)",
    home: "Home",
    searchPlaceholder: "Search bookmarks...",
    viewCurrent: "View current bookmarks",
    viewArchived: "View archives",
    settingsPrefix: "Settings:",
  },
  toolbox: {
    title: "Toolbox",
    back: "Back",
    empty: "No tools available",
    loading: "Loading...",
    tools: {
      aiTranslator: {
        name: "AI Translator",
        desc: "High-quality, multi-language text translation powered by AI with terminology retention."
      },
      aspectRatio: {
        name: "Aspect Ratio",
        desc: "Calculate common aspect ratios for images and videos with W1:H1 = W2:H2 linking."
      },
      timestamp: {
        name: "Timestamp Converter",
        desc: "Display current Unix timestamp and convert bidirectionally between timestamp and human-readable dates."
      },
      encoderDecoder: {
        name: "Base64 / URL Encode",
        desc: "Support string encoding and decoding for Base64 and URL."
      },
      colorConverter: {
        name: "Color Converter",
        desc: "Convert and preview between HEX, RGB, RGBA, and HSL color formats quickly."
      }
    },
    encoderDecoder: {
      copied: "Copied",
      input: "Input",
      output: "Output",
      inputPlaceholder: "Enter content here...",
      outputPlaceholder: "Result will appear here...",
      encode: "Encode",
      decode: "Decode",
      swap: "Swap",
      encodeFailed: "Encoding failed",
      decodeFailed: "Decoding failed",
      textMode: "Base64 (Text)",
      imageMode: "Base64 (Image)",
      imageToBase64: "Image to Base64",
      uploadHint: "Click or drag image here",
      changeImage: "Change image",
      copyBase64: "Copy Base64",
      base64ToImage: "Base64 to Image",
      pasteBase64Hint: "Paste Base64 string here (e.g. data:image/png;base64,...)",
      imagePreview: "Image Preview"
    },
    colorConverter: {
      copied: "Copied"
    },
    aiTranslator: {
      translateTo: "Translate to {lang}",
      translating: "Translating...",
      translate: "Translate",
      sourcePlaceholder: "Enter text to translate...",
      shortcutHint: "Ctrl + Enter to translate",
      targetPlaceholder: "Translation result will appear here...",
      copied: "Copied",
      copy: "Copy",
      failed: "Translation failed",
      prompt: "You are a professional translation engine. Please translate the following text into {lang}.\nRequirements:\n1. Return only the translation result without any explanations.\n2. Maintain original meaning and tone.\n3. Keep proper nouns unchanged.\n\nText to translate:\n{text}"
    },
    aspectRatio: {
      vertical: "(Vertical)",
      originalSize: "Original Size (W₁ × H₁)",
      newSize: "New Size (W₂ × H₂)",
      width: "Width",
      height: "Height",
      placeholder: "Enter number",
      custom: "Custom",
      customExample: "e.g., 2.35:1",
      classic: "Classic",
      ultrawide: "Ultrawide",
      square: "Square",
      portrait: "Portrait"
    },
    timestamp: {
      currentTime: "Current Timestamp",
      clickToCopy: "Click to Copy",
      toDateTime: "Timestamp to DateTime",
      toTimestamp: "DateTime to Timestamp",
      timestampPlaceholder: "Enter timestamp...",
      datePlaceholder: "e.g. 2026-07-08 14:11:37",
      invalidFormat: "Invalid Format",
      convertFailed: "Convert Failed",
      seconds: "Seconds",
      secondsShort: "s",
      milliseconds: "Milliseconds",
      millisecondsShort: "ms",
      switchUnit: "Switch Unit",
      copy: "Copy",
      stop: "Stop",
      start: "Start",
      convert: "Convert",
      result: "Result",
      localTime: "Local Time"
    }
  },
  vault: {
    title: "Vault",
    desc: "Centralized & encrypted storage for LLM API Keys, SecretPairs, Credentials & Passwords",
    setupTitle: "Initialize Vault",
    setupDesc: "Set a Master Password to encrypt and protect your credentials. Master Key is only stored in memory.",
    unlockTitle: "Unlock Vault",
    unlockDesc: "Enter Master Password to view and use your stored credentials",
    masterPassword: "Master Password",
    confirmMasterPassword: "Confirm Master Password",
    masterPasswordMinLength: "Master Password must be at least 6 characters",
    passwordMismatch: "Master Passwords do not match",
    unlock: "Unlock",
    lock: "Lock Vault",
    setupBtn: "Initialize & Unlock",
    searchPlaceholder: "Search provider, title or tags...",
    categories: {
      all: "All",
      ai_provider: "API Key",
      cloud_service: "Cloud Secret Pair",
      account_login: "Account Login",
      custom: "Custom Credential"
    },
    types: {
      api_key: "Single API Key",
      secret_pair: "SecretId / SecretKey",
      user_password: "Username / Password",
      custom: "Custom Credential"
    },
    newItem: "Add Credential",
    editItem: "Edit Credential",
    serviceName: "Provider / Platform",
    serviceNamePlaceholder: "e.g. OpenAI, DeepSeek, AWS",
    itemTitle: "Title / Description",
    itemTitlePlaceholder: "e.g. Translation Plugin Key, Production Key",
    category: "Category",
    credentialType: "Credential Type",
    apiKey: "API Key",
    apiKeyPlaceholder: "sk-...",
    secretId: "SecretId / AccessKeyId",
    secretKey: "SecretKey / AccessKeySecret",
    username: "Username / Account",
    password: "Password",
    websiteUrl: "Website URL",
    customFields: "Custom Fields",
    addCustomField: "Add Field",
    addFieldTitle: "Add Field",
    fieldType: "Field Type",
    fieldTypeText: "Text",
    fieldTypePassword: "Password",
    fieldTypeTextHint: "Use text fields for data such as security questions",
    fieldTypePasswordHint: "Use password fields for sensitive data like passwords or PINs",
    fieldLabel: "Field Label",
    fieldLabelPlaceholder: "e.g. Security Answer, API Endpoint",
    add: "Add",
    fieldName: "Field Name",
    fieldValue: "Field Value",
    notes: "Notes",
    notesPlaceholder: "e.g. Quota limit, reset cycle or security alerts",
    tags: "Tags (Comma separated)",
    copyKey: "Copy API Key",
    copySecretKey: "Copy SecretKey",
    copyPassword: "Copy Password",
    copySuccess: "Copied to clipboard",
    deleteConfirm: "Are you sure you want to delete this credential?",
    lockedHint: "Vault is locked",
    emptyHint: "No saved credentials. Click 「New Item」 to add."
  },
  agentRules: {
    title: "Agent Rules Hub",
    desc: "Centralized store for AI agent instructions and conventions (AGENTS.md, .cursorrules, CLAUDE.md, System Prompts)",
    searchPlaceholder: "Search rule title, tech stack or tags...",
    newRule: "New Rule",
    editRule: "Edit Rule",
    ruleTitle: "Rule Title",
    ruleTitlePlaceholder: "e.g. Vue 3 + Vite Guidelines, Cursor Global Rules",
    ruleType: "Specification Type",
    category: "Category",
    description: "Description / Use Case",
    descriptionPlaceholder: "Brief description of use case or target tool",
    content: "Rule Markdown Content",
    contentPlaceholder: "# Project Conventions\n\n## Code Style\n- TypeScript strict mode...",
    tags: "Tags (Comma separated)",
    tagsPlaceholder: "Vue3, TypeScript, Tailwind",
    categories: {
      all: "All",
      general: "General",
      frontend: "Frontend",
      backend: "Backend",
      fullstack: "Fullstack",
      devops: "DevOps"
    },
    types: {
      all: "All Types",
      agents_md: "AGENTS.md",
      cursorrules: ".cursorrules",
      windsurfrules: ".windsurfrules",
      claude_md: "CLAUDE.md",
      prompt: "Prompt",
      custom: "Custom"
    },
    copyContent: "Copy",
    copySuccess: "Rule content copied to clipboard",
    downloadFile: "Export File",
    deleteConfirm: "Are you sure you want to delete this rule?",
    emptyHint: "No agent rules found. Click 「New Rule」 to create one."
  }
};
